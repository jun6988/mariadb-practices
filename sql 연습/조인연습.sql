-- inner join
-- 예제1) 현재, 근무하고 있는 이름과 직책을 모두 출력 하세요.
select a.first_name, b.title
	from employees a, titles b -- alias 사용
...
-- 1) Natural Join
-- 두 테이블에 이름이 같은 공통 칼럼이 있으면 조인 조건을 
-- 명시하지 않아도 암묵적으로 조인이 됨
-- 관례상 맞지 않다.(위 방법을 사용하는 것을 추천한다)
select a.first_name, b.title
...
-- 실습문제1: 현재 회사 생활을 반영한 직원별 근무 부서를 사번, 직원 전체 이름, 근무 부서 형태로 출력해보세요.
...
select a.dept_name, avg(salary)
     ...
     and b.to_date = '9999-01-01' -- 밑으로는 row 선택조건
	 and c.to_date = '9999-01-01'
